...
public class SyncAllDataActivity extends BaseActivity {
    ...
    private void loadingPurchaseRequisition() {

        if (IFiveEngine.isNetworkAvailable (this)) {
            pDialog.show ();
            UserAPICall userAPICall = RetroFitEngine.getRetrofit ().create (UserAPICall.class);
            Call<onlineRequisitionAll> callEnqueue = userAPICall.purchasereqdetailedlist (sessionManager.getToken (this));
            callEnqueue.enqueue (new Callback<onlineRequisitionAll> () {
                @Override
                public void onResponse(Call<onlineRequisitionAll> call, Response<onlineRequisitionAll> response) {
                    uploadToRealmDB (response.body ());
                    if ((pDialog != null) && pDialog.isShowing ())
                        pDialog.dismiss ();
                }
                ...
                @Override
                public void onFailure(Call<onlineRequisitionAll> call, Throwable t) {
                    if ((pDialog != null) && pDialog.isShowing ())
                        pDialog.dismiss ();
                    Log.d ("throwing Error", t.getMessage ());
                    Toast.makeText (SyncAllDataActivity.this, "Sync incomplete", Toast.LENGTH_SHORT).show ();
    ...
    private void uploadToRealmDB(final onlineRequisitionAll body) {
        ...
        observable
                ...
                .subscribe (new Observer<Integer> () {
                    ...
                    public void onCompleted() {
                        realm.beginTransaction ();
                        onlineRequisitionAll allDatasResponse = realm.copyToRealm (body);
                        realm.commitTransaction ();
                        Toast.makeText (SyncAllDataActivity.this, "Purchase Requisition Sync Completed.", Toast.LENGTH_SHORT).show ();
                        loadingPurchaseEnquiry();
                    }
    ...
    private void loadingPurchaseEnquiry() {

        if (IFiveEngine.isNetworkAvailable (this)) {
            pDialog.show ();
            UserAPICall userAPICall = RetroFitEngine.getRetrofit ().create (UserAPICall.class);
            Call<PurchaseEnquiryAll> callEnqueue = userAPICall.purchaseenqdetailedlist (sessionManager.getToken (this));
            callEnqueue.enqueue (new Callback<PurchaseEnquiryAll> () {
                @Override
                public void onResponse(Call<PurchaseEnquiryAll> call, Response<PurchaseEnquiryAll> response) {
                    uploadToRealmDB (response.body ());
                    if ((pDialog != null) && pDialog.isShowing ())
                        pDialog.dismiss ();
                }
                ...
                @Override
                public void onFailure(Call<PurchaseEnquiryAll> call, Throwable t) {
                    if ((pDialog != null) && pDialog.isShowing ())
                        pDialog.dismiss ();
                    Log.d ("throwing Error", t.getMessage ());
                    Toast.makeText (SyncAllDataActivity.this, "Sync incomplete", Toast.LENGTH_SHORT).show ();
    ...
    private void uploadToRealmDB(final PurchaseEnquiryAll body) {
        ...
        observable
                ...
                .subscribe (new Observer<Integer> () {
                    ...
                    public void onCompleted() {
                        realm.beginTransaction ();
                        PurchaseEnquiryAll allDatasResponse = realm.copyToRealm (body);
                        realm.commitTransaction ();
                        Toast.makeText (SyncAllDataActivity.this, "Purchase enquiry Sync Completed.", Toast.LENGTH_SHORT).show ();
                        loadingPurchase();
                    }
    ...
    private void loadingPurchase() {

        if (IFiveEngine.isNetworkAvailable (this)) {
            pDialog.show ();
            UserAPICall userAPICall = RetroFitEngine.getRetrofit ().create (UserAPICall.class);
            Call<PurchaseOrderAll> callEnqueue = userAPICall.purchasedetailedlist (sessionManager.getToken (this));
            callEnqueue.enqueue (new Callback<PurchaseOrderAll> () {
                @Override
                public void onResponse(Call<PurchaseOrderAll> call, Response<PurchaseOrderAll> response) {
                    uploadToRealmDB (response.body ());
                    if ((pDialog != null) && pDialog.isShowing ())
                        pDialog.dismiss ();
                }
                ...
                @Override
                public void onFailure(Call<PurchaseOrderAll> call, Throwable t) {
                    if ((pDialog != null) && pDialog.isShowing ())
                        pDialog.dismiss ();
                    Log.d ("throwing Error", t.getMessage ());
                    Toast.makeText (SyncAllDataActivity.this, "Sync incomplete", Toast.LENGTH_SHORT).show ();
    ...
    private void uploadToRealmDB(final PurchaseOrderAll body) {
        ...
        observable
                ...
                .subscribe (new Observer<Integer> () {
                    ...
                    public void onCompleted() {
                        realm.beginTransaction ();
                        PurchaseOrderAll allDatasResponse = realm.copyToRealm (body);
                        realm.commitTransaction ();
                        Toast.makeText (SyncAllDataActivity.this, "Purchase Sync Completed.", Toast.LENGTH_SHORT).show ();
